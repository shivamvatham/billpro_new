...
export const calculateInvoiceTotal = (items: InvoiceItem[], shippingAmount: number = 0) => {
  const itemsTotal = items.reduce((sum, item) => {
    const calc = calculateItemTotal(item.price || 0, item.quantity || 0, item.discountPercentage || 0, item.tax || 0);
...
export const calculateItemTaxDetails = (
  ...
  if (isTaxable && product && taxConfig) {
    ...
    if (sameState) {
      ...
      const tax3Rate = product.productTax?.tax3Rate ?? taxConfig.tax3?.taxRate ?? 0;
      tax2 = { amount: afterDiscount * tax2Rate / 100, percentage: tax2Rate };
      tax3 = { amount: afterDiscount * tax3Rate / 100, percentage: tax3Rate };
    } else {
      const tax1Rate = product.productTax?.tax1Rate ?? taxConfig.tax1?.taxRate ?? 0;
