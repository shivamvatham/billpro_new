...
const calculateTaxRate = (product, taxConfig, customer, isTaxable) => {
  ...
  if (taxConfig?.taxType === "GST") {
    const isSameState =
      customer.gstNumber &&
      taxConfig?.taxNumber &&
      customer.gstNumber.substring(0, 2) ===
        taxConfig.taxNumber.substring(0, 2);
...
const validateTaxableInvoice = (salesSeries, taxConfig) => {
  if (
    salesSeries.invoiceTaxable &&
    (!taxConfig || taxConfig.taxType === "None")
  ) {
    throw new ApiError(
      400,
      "Tax configuration is required for taxable invoices"
    );
  }
};
